...
with open('ros_objekti_v2.txt', encoding='ansi') as f:
    lines = f.readlines()
    ...
    for sklop_name,sklop_content_outer in plotly_sklop_content_inner_instances.items():
        ...
        # Convert the plotly sklop_content_inner to relevant Gazebo SDF
            ...
            for i in range(len(x_values) - 1):
                ...
                # Dilate the line to create the thickness
                path = line.buffer(initial_buffer, cap_style=2, join_style=2)
                
                narrow_buffer = initial_buffer - 0.2  # Adjust the 0.2 value for desired edge thickness
                narrow_path = line.buffer(narrow_buffer, cap_style=2, join_style=2)
                edges_path = path.difference(narrow_path)
        
                # Generate XML elements for the main narrow path (gray)
                minx_n, miny_n, maxx_n, maxy_n = narrow_path.bounds
                width_n = maxx_n - minx_n
                length_n = maxy_n - miny_n
                height_n = 0.1  # As you've defined
                midpoint_x_n = (minx_n + maxx_n) / 2
                midpoint_y_n = (miny_n + maxy_n) / 2
                link = ET.SubElement(model, 'link', name=f"core_segment_{i}")
                pose = ET.SubElement(link, 'pose')
                pose.text = f"{midpoint_x_n} {midpoint_y_n} {height_n/2} 0 0 0"
                collision = ET.SubElement(link, 'collision', name=f"core_collision_{i}")
                visual = ET.SubElement(link, 'visual', name=f"core_visual_{i}")
                geometry_collision = ET.SubElement(collision, 'geometry')
                geometry_visual = ET.SubElement(visual, 'geometry')
                box_collision = ET.SubElement(geometry_collision, 'box')
                box_visual = ET.SubElement(geometry_visual, 'box')
                size_collision = ET.SubElement(box_collision, 'size')
                size_collision.text = f"{width_n} {length_n} {height_n}"
                size_visual = ET.SubElement(box_visual, 'size')
                size_visual.text = f"{width_n} {length_n} {height_n}"
                material = ET.SubElement(visual, 'material')
                ambient = ET.SubElement(material, 'ambient')
                ambient.text = "0.8627, 0.8627, 0.8627, 1.0"  # Gray
        
                # Generate XML elements for the edges (yellow)
                for j, region in enumerate(edges_path.geoms):
                    minx_e, miny_e, maxx_e, maxy_e = region.bounds
                    width_e = maxx_e - minx_e
                    length_e = maxy_e - miny_e
                    height_e = 0.2  # As you've defined
                    midpoint_x_e = (minx_e + maxx_e) / 2
                    midpoint_y_e = (miny_e + maxy_e) / 2
                    link = ET.SubElement(model, 'link', name=f"edge_segment_{i}_{j}")
                    pose = ET.SubElement(link, 'pose')
                    pose.text = f"{midpoint_x_e} {midpoint_y_e} {height_e/2} 0 0 0"
                    collision = ET.SubElement(link, 'collision', name=f"edge_collision_{i}_{j}")
                    visual = ET.SubElement(link, 'visual', name=f"edge_visual_{i}_{j}")
                    geometry_collision = ET.SubElement(collision, 'geometry')
                    geometry_visual = ET.SubElement(visual, 'geometry')
                    box_collision = ET.SubElement(geometry_collision, 'box')
                    box_visual = ET.SubElement(geometry_visual, 'box')
                    size_collision = ET.SubElement(box_collision, 'size')
                    size_collision.text = f"{width_e} {length_e} {height_e}"
                    size_visual = ET.SubElement(box_visual, 'size')
                    size_visual.text = f"{width_e} {length_e} {height_e}"
                    material = ET.SubElement(visual, 'material')
                    ambient = ET.SubElement(material, 'ambient')
                    # ambient.text = "1.0, 1.0, 0.0, 1.0"  # Yellow
                    ambient.text = "0.8627, 0.8627, 0.8627, 1.0"  # Gray
